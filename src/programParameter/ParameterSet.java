 ...
 * @since 2014-11-22
 */
public class ParameterSet<T extends String> {
	 ...
	 */
	public void parse(T[] inputs, int off, Appendable output) {
		outStream = output;
	...
	public void parseInteractive(T[] inputs, int off, BufferedReader input, Appendable output,
		...
		Map<T, Map.Entry<ParameterMetaData<T, ?>, Boolean>> parametersCompleted = new HashMap<>();
		for(ParameterMetaData<T, ?> parameter : parameters) {
			parametersCompleted.put(parameter.getPrimaryName(), Entries.newMutable(parameter, false));
		}
		int nextParamStart = -1;
		for(int i = paramStart+1, size = inputs.length; i < size; i++) {
			paramTemp = parameterNameMap.get(inputs[i]);
			if(paramTemp != null || (i == size - 1 && param != null)) {
				nextParamStart = i + (paramTemp == null && i == size - 1 ? 1 : 0);
				param.parse(inputs, paramStart, nextParamStart - paramStart);
				parametersCompleted.get(param.getPrimaryName()).setValue(true);
				paramStart = nextParamStart;
				param = paramTemp;
				if(param != null && i == size - 1) {
					param.parse(inputs, paramStart, 1);
		...
		for(Map.Entry<T, Map.Entry<ParameterMetaData<T, ?>, Boolean>> paramComplete : parametersCompleted.entrySet()) {
			if(paramComplete.getValue().getValue() == false && paramComplete.getValue().getKey().isRequired()) {
				@SuppressWarnings("unchecked")
				ParameterMetaData<String, ?> paramData = (ParameterMetaData<String, ?>) paramComplete.getValue().getKey();
	...
	private static final void getParameterInteractive(ParameterMetaData<String, ?> param,
		...
		try {
			...
			List<String> inputs = ParameterParser.parseParameters(line);
			inputs.add(0, param.getPrimaryName());
	 ...
	 */
	private final ParameterMetaData<T, ?> createHelpParameter(String helpParamName, String helpMsg,
